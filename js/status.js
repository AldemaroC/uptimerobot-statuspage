...
myApp.dashboard = (function($) {
  ...
  function init() {
    ...
    for (var d = 0; d <= 6; d++) {
      ...
      datestr = tmpdate.getDate() + "/" + (tmpdate.getMonth() + 1);
      $_servertitle.append("<th style=\"width:10%\">" + datestr + "</th>");
    }
    $_servertitle.append("<th style=\"width:9%\">Últimos 30 dias</th>");
  ...
  */
  ...
  function buildServerHTML(data, ids) {
    ...
    switch (parseInt(data.status, 10)) {
      ...
      case 1:
        ...
        break;
      case 2:
        data.statustext = "Operacional";
        data.statusicon = "ok";
        data.label = "success";
        data.alert = "";
        break;
      case 8:
        data.statustext = "Anormal";
    ...
    while (stat = bar.pop()) {
      ...
      if (stat.len == 1) {
        stattip += " (Últimas 24 horas)"
      } else {
        if (stat.right - stat.left < 1000 * 3540) {
          stattip += " (" + new Number((stat.right - stat.left) / (1000 * 60)).toFixed(0) + " Minuto)";
        } else {
          stattip += " (" + new Number((stat.right - stat.left) / (1000 * 3600)).toFixed(1) + " Hora)";
        }
    ...
    for (a = 0; a < uptimes.length; a++) {
      ...
      hours = minutes / 60;
      if (uptimes[a] >= 99.99) {
        uptimetext[a] = "100% de disponibilidade";
      } else if (minutes < 60) {
        uptimetext[a] = new Number(uptimes[a]).toFixed(2) + "% de disponibilidade<br>Falha de " + new Number(minutes).toFixed(0) + " minutos";
      } else {
        uptimetext[a] = new Number(uptimes[a]).toFixed(2) + "% de disponibilidade<br>Falha de " + new Number(hours).toFixed(1) + " horas";
      }
    }
  ...
  function Type2Word(t, icon) {
    switch (t) {
    case 1:
      return (icon ? "<span class=\"glyphicon glyphicon-remove-sign\"></span> " : "") + "Falha";
    case 2:
      return (icon ? "<span class=\"glyphicon glyphicon-ok-sign\"></span> " : "") + "Operacional";
      //case 99:
      //  return "Desconhecido";
      //case 98:
      //  return "Desconhecido";
    default:
      return (icon ? "<span class=\"glyphicon glyphicon-question-sign\"></span> " : "") + "Desconhecido";
    }
  }
